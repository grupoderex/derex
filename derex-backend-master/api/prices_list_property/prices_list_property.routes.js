 ...
 */
const PricesListProperty = require("./prices_list_property.sql");
...
} = require("./prices_list_property.schemas");
...
router.get(
  "/:id",
  checkSchema(getPricesListByIdSchema, ["params"]),
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: "Error al obtener lista de precio por ID." });
    }
  }
);
...
router.post(
  ...
  checkSchema(createPricesListSchema, ["body"]),
  validationMiddleware,
  getAdminSession,
  async (req, res) => {
...
router.put(
  "/update/:id",
  checkSchema(tokenHeadersSchema, ["headers"]),
  checkSchema(updatePricesListSchema, ["body", "params"]),
  validationMiddleware,
  getAdminSession,
  async (req, res) => {
    const { email, id: id_admin, name } = req.session;
...
router.delete(
  "/delete/:id",
  checkSchema(tokenHeadersSchema, ["headers"]),
  checkSchema(deletePricesListSchema, ["params"]),
  validationMiddleware,
  getAdminSession,
  async (req, res) => {
    const { email, id: id_admin, name } = req.session;
    const id = req.params.id;
    try {
      const result = await PricesListProperty.delete(id);
      await LogActivities.create({
        email,
        name,
        id_admin,
        activity: `Lista de precio eliminada id ${id}${
          result.schedulesDeleted > 0
            ? ` (${result.schedulesDeleted} programación(es) eliminada(s))`
            : ""
        }`,
      });
      res.json({
        mensaje: "Lista de precio eliminada exitosamente.",
...
router.put(
  ...
  checkSchema(tokenHeadersSchema, ["headers"]),
  checkSchema(updatePricesListByPropertyIdSchema, ["body", "params"]),
  validationMiddleware,
  getAdminSession,
  async (req, res) => {
    const { email, id: id_admin, name } = req.session;
