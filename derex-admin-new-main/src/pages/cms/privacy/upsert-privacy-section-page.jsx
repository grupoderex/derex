...
export function UpsertPrivacySectionPage({ isReadOnly = false, isEdit = false, isCreate = false }) {
  const { dataAuth } = useAppContext();

  const { id } = useParams();
  const router = useNavigate();

  const { refetch, isLoading } = useQuery('notice-section', {
  ...
  const { mutate: create, isLoading: loadingCreate } = useMutation('createnotice-section', {
    mutationFn: (values) =>
      api_createDecalogueSection(
        {
          ...values,
  ...
  const { mutate: update, isLoading: loadingUpdate } = useMutation('updatenotice-section', {
    mutationFn: (values) =>
      api_updateDecalogueSection(
        id,
        {
  ...
  return (
    <Container>
      <h1>Sección de avisos de privacidad</h1>
      <Card>
        <CardContent>
          <div>
            <form
...
UpsertPrivacySectionPage.propTypes = {
